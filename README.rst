...
-  5 functions on Window operations (works only in Windows OS). Type
   cf.window
...
-  Loads of miscellaneous functions related to OCR, Send WhatsApp
   Message, Send Email using Outlook Desktop app (works only in Windows
   OS), emoji, capture photo, flash (pop-up) messages etc.

ClointFusion's function works in different modes:
=================================================

1) If you pass all the required parameters, function works silently. So,
   this is Non-GUI mode.
...
Contribute by giving us a Star / writing article on ClointFusion / feedback
...
Drop a mail to ClointFusion@cloint.com or ClointFusion@gmail.com
